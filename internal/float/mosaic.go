...
func Sample() Rec {
	return Rec{Title: "glassfloat-sample", Body: "Float=TG01-12 level=0.5 epoch=4", Tags: []string{"tide"}}
}

func Seed() []Rec {
	return []Rec{
		Sample(),
		{Title: "glassfloat-seed-b", Body: "Float=TG01-12 level=0.5 epoch=4 b", Tags: []string{"tide"}},
	}
}
...
func Enforce(title, body string, tags []string) error {
	if strings.TrimSpace(title) == "" {
		return fmt.Errorf("Float title required")
	}
	if strings.TrimSpace(body) == "" {
		return fmt.Errorf("Float body required")
	}
	if len(tags) == 0 {
		return fmt.Errorf("Float tag required")
	}
	return nil
}
