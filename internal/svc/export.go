...
import (
	...
	"example.com/glassfloat/internal/util/filedump"
	"example.com/glassfloat/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "GlassFloat Bath", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "glassfloat"},
	}
